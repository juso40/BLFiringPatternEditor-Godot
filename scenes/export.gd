...
const EXPORT_TEMPLATE: String = (
    "(StartPoint=(Pitch={%d},Yaw={%d},Roll=0),EndPoint=(Pitch=0,Yaw=0,Roll=0),\
...
func update_export_command(markers: Array[Control]) -> void:
    if markers.is_empty():
    ...
    for marker in markers:
        export_command += EXPORT_TEMPLATE % [(marker.position.x - CENTER_X) * MAGIC_MULTIPLIER, (CENTER_Y-marker.position.y) * MAGIC_MULTIPLIER]
